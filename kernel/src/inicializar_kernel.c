...
void inicializar_kernel(){
    ...
    listaNuevosProcesos = list_create();
    listaProcesosReady = list_create();
    listaProcesosTerminados = list_create();
    list_interfaz = list_create();
...
void inicializar_semaforos(){
    sem_init(&existenProcesosNuevos, 0, 0);
    sem_init(&cantProcesosConcurrentes, 0, GRADO_MULTIPROGRAMACION); 
    sem_init(&existeProcesoReady, 0, 0);
    sem_init(&runningLibre, 0, 1);
    sem_init(&existeProcesoRunning, 0, 0);
    sem_init(&planiCorto, 0, 1);
    sem_init(&planiLargo, 0, 1);
    sem_init(&cambio_multi, 0, 0);
    pthread_mutex_init(&mutexNuevosProcesos, NULL);
    pthread_mutex_init(&mutexListaReady, NULL);
    pthread_mutex_init(&cambio_multi_mutex, NULL);
